...
export default function PaymentLinkModal({
  ...
  const [copyOne, setCopyOne] = useState(false);
  const [copyTwo, setCopyTwo] = useState(false);
    ...
    >
      <div className="h-auto flex gap-1  flex-col justify-center items-center w-1/3 md:w-full md:mx-2 bg-white relative p-6 pb-8 rounded-md">
        ...
        <p className="text-[#555555] text-left">
          Copy this link and send it to the people from whom you want to receive
          payment. Also, don&apos;t forget to save it in case you want to use it
          later
        </p>
        <div className="text-[#555] font-semibold mb-2 text-xs text-start w-full">
          Payment Id
        </div>
        {/* Search */}
        <div className="bg-[#F7F5FF] border rounded-md p-2 px-3 w-full flex justify-between">
          <input
            type="text"
            value={paymentId}
          ...
          {copyOne ? (
            ...
            <svg
              onClick={() => {
                copyCode(paymentId), setCopyOne(true);
              }}
        ...
        {/* Search */}
        <div className="bg-[#F7F5FF] border rounded-md p-2 px-3 flex w-full justify-between">
          <input
            type="text"
            value={paymentLink}
            className="bg-transparent outline-none text-sm px-2 w-full text-[#323232]"
          ...
          {copyTwo ? (
            ...
            <svg
              onClick={() => {
                copyCode(paymentLink), setCopyTwo(true);
              }}
              className="cursor-pointer"
              width="24"
              height="24"
