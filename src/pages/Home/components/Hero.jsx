...
const Hero = () => {
	const [transactModalOpen, setTransactModalOpen] =
		useState(false);
...
useEffect(() => {
  ...
  if (responseRecieved) {
    try {
      ...
      socket.on('connect', () => {
        console.log('Connection established with the server', `Payment${paymentDetails?.accountId}`);
      });
			...
			<section className='hero-section py-[60px] flex md:flex-col justify-between gap-[60px] sm:mt-[50px] sm:py-0'>
				<div className='left-side max-w-[589px] flex-[1.3]'>
					...
					<p className='py-5 font-ui-regular text-[16px] text-body-text md:text-left sm:text-[16px] sm:text-left'>
						Experience effortless payment processing through
						our innovative Escrow Approach. We simplify and
						secure transactions, ensuring peace of mind for
						both senders and receivers. Say goodbye to
						complexity and embrace a straightforward, secure
