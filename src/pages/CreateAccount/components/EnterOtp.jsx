...
const EnterOtp = ({ setStep }) => {
  ...
  const resendOtp = async () => {
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        console.log("OTP resend failed with status:", response.status);
      }
    } catch (error) {
      console.error("Error:", error);
    }
  ...
  return (
    <Container>
      ...
      <button
        onClick={() => verifyOtp()}
        className="bg-primary px-[64px] m-auto sm:px-[0px] py-[14px] my-[5px] w-fit sm:w-full font-ui-semi text-[16px] text-[#ffffff] rounded-[200000px]"
      >
